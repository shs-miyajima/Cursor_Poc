...
// E2E-dsp-008: ステータスバッジ return_requested
test('返却申請中のステータスが色分けバッジで表示される', async ({ page, request }) => {
  const created = await apiCreate(request, {
    mock_user_id: USERS.sato,
    equipment_id: EQUIPMENTS.windowsPc,
    requested_from: dateFromToday(5),
    requested_to: dateFromToday(6),
  });
  const loanId = (await created.json()).item.id;
  await apiUpdateStatus(request, loanId, { mock_user_id: USERS.kanri, status: 'approved' });
  await apiUpdateStatus(request, loanId, { mock_user_id: USERS.sato, status: 'return_requested' });

  await openPage(page);
  await selectUser(page, USERS.kanri);

  const badge = page.locator('[data-testid="status-badge"][data-status="return_requested"]').first();
  await expect(badge).toBeVisible();
  await expect(badge).toHaveText('返却申請中');
});

// E2E-dsp-009: 返却申請ボタン表示 本人 貸出中
test('一般社員は自分の貸出中の申請に返却申請ボタンが表示される', async ({ page }) => {
  // Seeder: 山田 太郎の iPhone 15 Pro が approved（期限切れ）
  await openPage(page);

  const approvedRow = page.locator('[data-testid="loan-row"]', {
    has: page.locator('[data-testid="status-badge"][data-status="approved"]'),
  });
  await expect(approvedRow.locator('[data-testid="return-request-button"]')).toBeVisible();
});

// E2E-dsp-010: 返却申請ボタン非表示 貸出中以外
test('一般社員は貸出中以外の自分の申請には返却申請ボタンが表示されない', async ({ page, request }) => {
  const returnedLoan = await apiCreate(request, {
    mock_user_id: USERS.yamada,
    equipment_id: EQUIPMENTS.macbook,
    requested_from: dateFromToday(5),
    requested_to: dateFromToday(6),
  });
  const returnedId = (await returnedLoan.json()).item.id;
  await apiUpdateStatus(request, returnedId, { mock_user_id: USERS.kanri, status: 'approved' });
  await apiUpdateStatus(request, returnedId, { mock_user_id: USERS.kanri, status: 'returned' });

  const rejectedLoan = await apiCreate(request, {
    mock_user_id: USERS.yamada,
    equipment_id: EQUIPMENTS.macbook,
    requested_from: dateFromToday(8),
    requested_to: dateFromToday(9),
  });
  const rejectedId = (await rejectedLoan.json()).item.id;
  await apiUpdateStatus(request, rejectedId, { mock_user_id: USERS.kanri, status: 'rejected' });

  const returnRequestedLoan = await apiCreate(request, {
    mock_user_id: USERS.yamada,
    equipment_id: EQUIPMENTS.macbook,
    requested_from: dateFromToday(11),
    requested_to: dateFromToday(12),
  });
  const returnRequestedId = (await returnRequestedLoan.json()).item.id;
  await apiUpdateStatus(request, returnRequestedId, { mock_user_id: USERS.kanri, status: 'approved' });
  await apiUpdateStatus(request, returnRequestedId, { mock_user_id: USERS.yamada, status: 'return_requested' });

  // Seeder 投入済みの approved（iPhone、期限切れ）は本人所有のため返却申請ボタン表示対象として別枠で確認する
  const approvedRow = page.locator('[data-testid="loan-row"]', {
    has: page.locator('[data-testid="status-badge"][data-status="approved"]'),
  });
  const nonApprovedRows = page.locator('[data-testid="loan-row"]').filter({
    hasNot: page.locator('[data-testid="status-badge"][data-status="approved"]'),
  });

  await openPage(page);

  await expect(approvedRow).toHaveCount(1);
  await expect(nonApprovedRows).toHaveCount(4);
  await expect(nonApprovedRows.locator('[data-testid="return-request-button"]')).toHaveCount(0);
});

// E2E-dsp-011: 返却申請ボタン非表示 他人の申請（管理者視点）
test('管理者が他ユーザーの貸出中の申請を閲覧しても返却申請ボタンは表示されない', async ({ page, request }) => {
  const created = await apiCreate(request, {
    mock_user_id: USERS.sato,
    equipment_id: EQUIPMENTS.windowsPc,
    requested_from: dateFromToday(5),
  ...
  const satoApprovedRow = page.locator('[data-testid="loan-row"]', {
    has: page.locator('[data-testid="status-badge"][data-status="approved"]'),
  }).filter({ hasText: '佐藤 花子' });
  await expect(satoApprovedRow.locator('[data-testid="status-button-approved"]')).toBeVisible();
  await expect(satoApprovedRow.locator('[data-testid="return-request-button"]')).toHaveCount(0);
});
...
// E2E-auth-007: 一般社員 返却申請可
test('一般社員が自分の貸出中の申請に返却申請するとステータスが返却申請中になる', async ({ page }) => {
  await openPage(page);

  const approvedRow = page.locator('[data-testid="loan-row"]', {
    has: page.locator('[data-testid="status-badge"][data-status="approved"]'),
  });
  await approvedRow.locator('[data-testid="return-request-button"]').click();

  await expect(
    page.locator('[data-testid="status-badge"][data-status="return_requested"]'),
  ).toHaveCount(1);
  await expect(
    page.locator('[data-testid="status-badge"][data-status="approved"]'),
  ).toHaveCount(0);
...
// E2E-auth-008: 一般社員 他人の申請へ返却申請不可
test('一般社員が他ユーザーの申請へ返却申請すると403で更新されない', async ({ request }) => {
  const created = await apiCreate(request, {
    mock_user_id: USERS.yamada,
    equipment_id: EQUIPMENTS.macbook,
    requested_from: dateFromToday(5),
  ...
  const response = await apiUpdateStatus(request, loanId, {
    mock_user_id: USERS.sato,
  ...
  expect(response.status()).toBe(403);
  const body = await response.json();
  expect(body.message).toBe('自分の申請のみ返却申請できます');

  const list = await apiList(request, { mock_user_id: USERS.kanri });
  const items = (await list.json()).items as { id: number; status: string }[];
  expect(items.find((i) => i.id === loanId)?.status).toBe('approved');
});

// E2E-auth-009: 一般社員 貸出中以外の自分の申請へ返却申請不可
test('一般社員が貸出中以外の自分の申請へ返却申請すると422で更新されない', async ({ request }) => {
  // 山田 太郎の Seeder 投入済み pending 申請（id=1）を利用する
  const response = await apiUpdateStatus(request, 1, {
    mock_user_id: USERS.yamada,
    status: 'return_requested',
  });
  ...
  expect(response.status()).toBe(422);
  const body = await response.json();
  expect(body.message ?? body.errors?.status?.[0]).toContain('現在のステータスでは返却申請できません');
...
// E2E-auth-010: 管理者 返却申請中を返却確定可
test('管理者が返却申請中の申請を返却操作で返却済に更新できる', async ({ page, request }) => {
  const created = await apiCreate(request, {
    mock_user_id: USERS.yamada,
    equipment_id: EQUIPMENTS.macbook,
    requested_from: dateFromToday(5),
  ...
  const loanId = (await created.json()).item.id;
  await apiUpdateStatus(request, loanId, { mock_user_id: USERS.kanri, status: 'approved' });
  await apiUpdateStatus(request, loanId, { mock_user_id: USERS.yamada, status: 'return_requested' });

  await openPage(page);
  await selectUser(page, USERS.kanri);

  const targetRow = page.locator(`[data-testid="loan-row"][data-loan-id="${loanId}"]`);
  await targetRow.locator('[data-testid="status-button-returned"]').click();

  await expect(
    page.locator(`[data-testid="loan-row"][data-loan-id="${loanId}"] [data-testid="status-badge"]`),
  ).toHaveAttribute('data-status', 'returned');
});

// E2E-auth-011: 管理者 返却申請中を差し戻し可
test('管理者が返却申請中の申請を承認操作で貸出中に差し戻せる', async ({ page, request }) => {
  const created = await apiCreate(request, {
    mock_user_id: USERS.yamada,
    equipment_id: EQUIPMENTS.macbook,
    requested_from: dateFromToday(5),
  ...
  const targetRow = page.locator(`[data-testid="loan-row"][data-loan-id="${loanId}"]`);
  await targetRow.locator('[data-testid="status-button-approved"]').click();
...
// E2E-dyn-006: 返却申請中は重複期間チェック対象
test('返却申請中の申請と重複する期間の申請はエラーになり登録されない', async ({ page, request }) => {
  const created = await apiCreate(request, {
    mock_user_id: USERS.yamada,
    equipment_id: EQUIPMENTS.macbook,
    requested_from: dateFromToday(20),
...
// E2E-dyn-007: 返却申請中は在庫数チェック対象
test('返却申請中を含めた在庫数を超える期間の申請はエラーになり登録されない', async ({ page, request }) => {
  // iPhone（在庫 1）を return_requested にして在庫を占有させる
  const created = await apiCreate(request, {
    mock_user_id: USERS.sato,
    equipment_id: EQUIPMENTS.iphone,
    requested_from: dateFromToday(20),
...
// E2E-dyn-008: 返却申請中でも期限切れアラート対象
test('返却申請中でも返却期限超過ならアラートと注意アイコンが表示される', async ({ page }) => {
  // 佐藤 花子に返却期限超過の return_requested 申請を DB 直接投入する（過去日は API では作成不可のため）
  runSql(
    "INSERT INTO equipment_loan_requests (user_id, equipment_id, status, requested_from, requested_to, reason, created_at, updated_at) VALUES (2, 3, 'return_requested', CURRENT_DATE - 8, CURRENT_DATE - 2, NULL, NOW(), NOW());",
  );
...
test('一覧 API のレスポンスに必要なキーが含まれる', async ({ request }) => {
  ...
  expect(body.items[0]).toHaveProperty('can_request_return');
});
