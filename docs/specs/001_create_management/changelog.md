...
- フェーズ: 実装・テスト
- 操作: draft
- 内容: 機能改修 #1「返却申請フロー追加」のフェーズ 4 実装完了。Enum / Model / Service /
  Controller / JS を改修し、PHPUnit 40 件（新規 9 件含む）・Vitest 25 件（新規 3 件含む）・
  Playwright E2E 49 件（新規 12 件含む）が全件成功。PHPUnit カバレッジ 89.7%（基準 80% 以上）、
  Vitest 対象モジュール 100%（基準 90% 以上）。`04-completion-report.md` を作成し
  `effort-report.md` §4.2/§4.3 に実績・削減効果を記録。

## 2026-07-06

- フェーズ: テスト設計
- 操作: approved
- 内容: 返却申請フロー追加（機能改修）についてユーザーの明示的な承認（選択式確認で「承認する」を選択）により `03-test-plan.status` を approved に更新。フェーズ 4（実装・テスト改修）に着手する。

## 2026-07-06

- フェーズ: テスト設計
- 操作: reopened
- 理由: フェーズ2改修（返却申請フロー追加）に伴い、E2E / PHPUnit / Vitest にケース追加が必要なため `03-test-plan.status` を `draft` に差し戻す。
- 対応: `03-test-plan.md` と 3 種の CSV に返却申請関連ケースを追加する。

## 2026-07-06

- フェーズ: 設計
- 操作: approved
- 内容: 返却申請フロー追加（機能改修）についてユーザーの明示的な承認（選択式確認で「承認する」を選択）により `02-design.status` を approved に更新。フェーズ 3（テスト設計改修）に着手する。

## 2026-07-06

- フェーズ: 設計
- 操作: reopened
- 理由: フェーズ1改修（返却申請フロー追加）が Service / Enum / Presenter / View に影響するため、`02-design.status` を `draft` に差し戻す。
- 対応: `02-design.md` に返却申請関連の設計を追記する。

## 2026-07-06

- フェーズ: 仕様整理
- 操作: approved
- 内容: 返却申請フロー追加（機能改修）についてユーザーの明示的な承認（選択式確認で「承認する」を選択）により `01-requirements.status` を approved に更新。棲み分け（既存 spec 改修 vs 新規 spec 作成）について質問があり、既存機能の延長のため `001_create_management` 内を改修する方針と回答した。フェーズ 2（設計改修）に着手する。

## 2026-07-06

- フェーズ: 仕様整理
- 操作: reopened
- 理由: 機能改修依頼。承認済み申請（貸出中）に対して一般ユーザーが返却申請を行えるフローを追加する。従来の「管理者が直接返却済に更新する」フローは維持する。
- 対応: `01-requirements.status` を `draft` に差し戻し、`01-requirements.md` を更新する。影響が及ぶ場合は `02-design.status` / `03-test-plan.status` も差し戻す。
